...
ShellRoot {
    ...
    property int cpuUsage: 0; property int cpuTemp: 0; property bool showCpuTemp: false
    property int gpuUsage: 0; property int gpuTemp: 0; property bool isNvidia: true; property bool showGpuTemp: false
    property bool showMemPercent: true; property int memUsagePercent: 0; property string memText: "0%"
    property bool showDiskPercent: true; property var disks: []; property int currentDiskIdx: 0; property string diskUsage: "0%"; property string diskLabel: "/"
    property string kernelVersion: "..."
    property string wifiSSID: "Offline"; property int wifiStrength: 0
    ...
    Process {
        id: kernelProc; command: ["uname", "-r"]; running: true
    ...
    Process {
        id: gpuProc
        command: root.isNvidia ? ["sh", "-c", "nvidia-smi --query-gpu=utilization.gpu,temperature.gpu --format=csv,noheader,nounits"]
        ...
        stdout: SplitParser {
            onRead: data => {
                ...
                if (parts.length >= 2) { root.gpuUsage = parseInt(parts[0].trim()) || 0; root.gpuTemp = parseInt(parts[1].trim()) || 0 }
                else if (parts.length === 1) { root.gpuUsage = parseInt(parts[0].trim()) || 0 }
            }
        }
    }
    ...
    Process {
        ...
        stdout: SplitParser {
            onRead: d => {
                ...
                let usedMb = parseInt(parts[0]); let totalMb = parseInt(parts[1])
    ...
    Process {
        id: wifiProc; command: ["nmcli", "-t", "-f", "active,ssid,signal", "dev", "wifi"]
        stdout: SplitParser {
            onRead: data => {
                if (data && data.startsWith("yes")) {
                    let parts = data.trim().split(":")
                    wifiSSID = parts[1] || "Unknown"; wifiStrength = parseInt(parts[2]) || 0
                } else if (!wifiSSID || wifiSSID === "") { wifiSSID = "Offline"; wifiStrength = 0 }
            }
        }
    }
                    ...
                    RowLayout {
                        ...
                        Item {
                            ...
                            Row {
                                ...
                                Text { text: "󰌽"; color: root.cal9; font.pixelSize: root.fontSize + 2; font.family: root.fontFamily; anchors.verticalCenter: parent.verticalCenter }
                        ...
                        Item {
                            ...
                            Row {
                                ...
                                Text { text: ""; color: root.cal11; font.pixelSize: root.fontSize + 2; font.family: root.fontFamily; anchors.verticalCenter: parent.verticalCenter }
                                Item { height: 20; width: parent.expanded ? cpuTxt.implicitWidth + 8 : 0; clip: true; anchors.verticalCenter: parent.verticalCenter; Behavior on width { NumberAnimation { duration: 300; easing.type: Easing.OutCubic } }
                                    Text { id: cpuTxt; anchors.left: parent.left; anchors.leftMargin: 6; anchors.verticalCenter: parent.verticalCenter; text: root.showCpuTemp ? root.cpuTemp + "°C" : root.cpuUsage + "%"; color: root.cal11; font.pixelSize: root.fontSize; font.family: root.fontFamily; opacity: parent.width > 5 ? 1 : 0; Behavior on opacity { NumberAnimation { duration: 200 } } } }
                            }
                            MouseArea { anchors.fill: parent; cursorShape: Qt.PointingHandCursor; acceptedButtons: Qt.LeftButton | Qt.MiddleButton; hoverEnabled: true; onEntered: cpuRow.hovered = true; onExited: cpuRow.hovered = false; onClicked: (m) => { if(m.button === Qt.MiddleButton) cpuRow.pinned = !cpuRow.pinned; else if (m.button === Qt.LeftButton) root.showCpuTemp = !root.showCpuTemp } }
                        }
                        Rectangle { width: 1; height: 12; color: root.cal3 }
                        ...
                        Item {
                            ...
                            Row {
                                ...
                                Text { text: "󰢮"; color: root.isNvidia ? root.cal13 : root.cal7; font.pixelSize: root.fontSize + 2; font.family: root.fontFamily; anchors.verticalCenter: parent.verticalCenter }
                                Item { height: 20; width: parent.expanded ? gpuContent.implicitWidth + 8 : 0; clip: true; anchors.verticalCenter: parent.verticalCenter; Behavior on width { NumberAnimation { duration: 300; easing.type: Easing.OutCubic } }
                                    Row { id: gpuContent; anchors.left: parent.left; anchors.leftMargin: 6; anchors.verticalCenter: parent.verticalCenter; spacing: 6; opacity: parent.width > 5 ? 1 : 0; Behavior on opacity { NumberAnimation { duration: 200 } }
                                        Rectangle { width: 24; height: 16; radius: 4; color: root.isNvidia ? root.cal13 : root.cal7; anchors.verticalCenter: parent.verticalCenter; Text { anchors.centerIn: parent; text: root.isNvidia ? "NV" : "IN"; color: root.cal0; font.pixelSize: 10; font.family: root.fontFamily; font.bold: true } }
                                        Text { text: root.showGpuTemp ? root.gpuTemp + "°C" : root.gpuUsage + "%"; color: root.isNvidia ? root.cal13 : root.cal7; font.pixelSize: root.fontSize; font.family: root.fontFamily; anchors.verticalCenter: parent.verticalCenter }
                                    }
                                }
                            }
                            MouseArea { anchors.fill: parent; cursorShape: Qt.PointingHandCursor; acceptedButtons: Qt.LeftButton | Qt.MiddleButton | Qt.RightButton; hoverEnabled: true; onEntered: gpuRow.hovered = true; onExited: gpuRow.hovered = false; onClicked: (m) => { if(m.button === Qt.MiddleButton) gpuRow.pinned = !gpuRow.pinned; else if (m.button === Qt.LeftButton) { root.isNvidia = !root.isNvidia; gpuProc.running = false; gpuProc.running = true } else if (m.button === Qt.RightButton) { root.showGpuTemp = !root.showGpuTemp } } }
                        }
                        Rectangle { width: 1; height: 12; color: root.cal3 }
                        ...
                        Item {
                            ...
                            Row {
                                ...
                                Text { text: ""; color: root.cal13; font.pixelSize: root.fontSize + 2; font.family: root.fontFamily; anchors.verticalCenter: parent.verticalCenter }
                        ...
                        Item {
                            ...
                            Row {
                                ...
                                Text { text: "󰋊"; color: root.cal7; font.pixelSize: root.fontSize + 2; font.family: root.fontFamily; anchors.verticalCenter: parent.verticalCenter }
                                Item { height: 20; width: parent.expanded ? diskTxt.implicitWidth + 8 : 0; clip: true; anchors.verticalCenter: parent.verticalCenter; Behavior on width { NumberAnimation { duration: 300; easing.type: Easing.OutCubic } }
                                    Text { id: diskTxt; anchors.left: parent.left; anchors.leftMargin: 6; anchors.verticalCenter: parent.verticalCenter; text: root.diskLabel + ": " + root.diskUsage; color: root.cal7; font.pixelSize: root.fontSize; font.family: root.fontFamily; opacity: parent.width > 5 ? 1 : 0; Behavior on opacity { NumberAnimation { duration: 200 } } } }
                            }
                            MouseArea { anchors.fill: parent; cursorShape: Qt.PointingHandCursor; acceptedButtons: Qt.LeftButton | Qt.MiddleButton | Qt.RightButton; hoverEnabled: true; onEntered: diskRow.hovered = true; onExited: diskRow.hovered = false; onClicked: (m) => { if(m.button === Qt.MiddleButton) diskRow.pinned = !diskRow.pinned; else if (m.button === Qt.LeftButton && root.disks.length > 0) { root.currentDiskIdx = (root.currentDiskIdx + 1) % root.disks.length; root.updateDiskText() } else if (m.button === Qt.RightButton) { root.showDiskPercent = !root.showDiskPercent; root.updateDiskText() } } }
                        }
                        ...
                        Item {
                            ...
                            Row {
                                ...
                                Text { text: parent.icon; color: root.wifiSSID === "Offline" ? root.cal11 : root.cal10; font.pixelSize: root.fontSize + 2; font.family: root.fontFamily; anchors.verticalCenter: parent.verticalCenter }
                                Item { height: 20; width: parent.expanded ? wifiTxt.implicitWidth + 8 : 0; clip: true; anchors.verticalCenter: parent.verticalCenter; Behavior on width { NumberAnimation { duration: 300; easing.type: Easing.OutCubic } }
                                    Text { id: wifiTxt; anchors.left: parent.left; anchors.leftMargin: 6; anchors.verticalCenter: parent.verticalCenter; text: (root.wifiSSID === "Offline" ? "Searching..." : root.wifiSSID) + " (" + root.wifiStrength + "%)"; color: root.wifiSSID === "Offline" ? root.cal11 : root.cal10; font.pixelSize: root.fontSize; font.family: root.fontFamily; opacity: parent.width > 5 ? 1 : 0; Behavior on opacity { NumberAnimation { duration: 200 } } } }
                            }
                            MouseArea {
