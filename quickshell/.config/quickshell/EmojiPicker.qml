...
PanelWindow {
    ...
    visible: active
		...
		property string emojiFile: StandardPaths.writableLocation(StandardPaths.HomeLocation).toString().replace("file://", "") + "/.cache/emoji-picker/emoji.txt"
    ...
    FileView {
        ...
        onLoaded: {
            var content = text()
            if (content !== "") {
                root.allEmojis = content.trim().split("\n")
            }
        }
    }

    readonly property var filteredEmojis: {
        if (searchText === "") return allEmojis
            ...
            ListView {
                ...
                delegate: Rectangle {
                    ...
                    MouseArea {
                        ...
                        onClicked: root.copyEmoji(modelData.split(" ")[0])
                    }
                }
            }
